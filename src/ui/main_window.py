...
class _LeftPanel(QWidget):
    ...
    def dropEvent(self, event) -> None:
        if event.mimeData().hasFormat(MIME_PRIORITY_ITEM):
            try:
                item_key = str(event.mimeData().data(MIME_PRIORITY_ITEM).data().decode() or "")
                if item_key:
...
class MainWindow(QMainWindow):
    ...
    def _connect_signals(self) -> None:
        self._priority_panel.priority_list.items_changed.connect(self._on_priority_items_changed)
        self._priority_panel.priority_list.manual_action_rename_requested.connect(self._on_rename_manual_action)
        self._priority_panel.priority_list.manual_action_rebind_requested.connect(self._on_rebind_manual_action)
        self._priority_panel.priority_list.manual_action_remove_requested.connect(self._on_remove_manual_action)
        self._priority_panel.add_manual_action_requested.connect(self._on_add_manual_action)
    ...
    def _active_priority_items(self) -> list[dict]:
        ...
        return [{"type": "slot", "slot_index": i} for i in self._active_priority_order()]
    ...
    def _set_priority_list_from_active_profile(self) -> None:
        ...
        try:
            self._priority_panel.priority_list.set_manual_actions(self._active_manual_actions())
    ...
    def _next_priority_candidate(self, states: list[dict]) -> Optional[dict]:
        ...
        manual_by_id = {
            str(a.get("id", "") or "").strip().lower(): a for a in self._active_manual_actions()
        }
    ...
    def _next_casting_priority_slot(self, states: list[dict]) -> tuple[Optional[int], Optional[float]]:
        """First prioritized slot currently casting/channeling and its cast_ends_at."""
        by_index = {s["index"]: s for s in states}
        for item in self._active_priority_items():
    ...
    def _on_add_manual_action(self) -> None:
        ...
        keybind, ok = QInputDialog.getText(self, "Add Manual Action", "Keybind to send:")
        if not ok:
            return
        ...
        existing_ids = {str(a.get("id", "") or "").strip().lower() for a in actions if isinstance(a, dict)}
        i = 1
        ...
        if not items:
            items = [{"type": "slot", "slot_index": i} for i in list(profile.get("priority_order", []))]
    ...
    def _on_rename_manual_action(self, action_id: str) -> None:
        ...
        name, ok = QInputDialog.getText(self, "Rename Manual Action", "Action name:", text=current)
        if not ok:
            return
    ...
    def _on_rebind_manual_action(self, action_id: str) -> None:
        ...
        keybind, ok = QInputDialog.getText(self, "Rebind Manual Action", "Keybind to send:", text=current)
        if not ok:
            return
        keybind = normalize_bind((keybind or "").strip())
        if not keybind:
    ...
    def _on_remove_manual_action(self, action_id: str) -> None:
        ...
        profile = self._config.get_active_priority_profile()
        actions = [
            a for a in list(profile.get("manual_actions", []))
            if str(a.get("id", "") or "").strip().lower() != aid
        ]
        items = [
            i for i in list(profile.get("priority_items", []))
            if not (
    ...
    def update_slot_states(self, states: list[dict]) -> None:
        ...
        next_item = self._next_priority_candidate(states)
        if next_item is not None:
            keybind = str(next_item.get("keybind", "") or "").strip() or "?"
            slot_name = str(next_item.get("display_name", "") or "").strip() or "Unidentified"
            if not self._config.automation_enabled:
                suffix = "ready (paused)"
                color = KEY_YELLOW
